...
int main(int argc, char** argv){
    ...
    auto load_swc = [&](const std::string& filename){
        ...
        auto swc_pts = swc_file.GetAllPoints();
        {
            ...
            for(auto& pt : swc_pts){
                bounds |= Float3(pt.x - pt.radius, pt.y - pt.radius, pt.z - pt.radius);
                bounds |= Float3(pt.x + pt.radius, pt.y + pt.radius, pt.z + pt.radius);
            }
            LOG_INFO("swc bounds is: {} {} {}, {} {} {}", bounds.low.x, bounds.low.y, bounds.low.z,
                     bounds.high.x, bounds.high.y, bounds.high.z);
            auto voxel_shape = (bounds.high - bounds.low) / volume_info.voxel_space;
            voxel_shape.x = ceil(voxel_shape.x), voxel_shape.y = ceil(voxel_shape.y), voxel_shape.z = ceil(voxel_shape.z);
            LOG_INFO("swc voxel shape: {} {} {}",
                     int(voxel_shape.x), int(voxel_shape.y), int(voxel_shape.z));


//            for(auto& pt : swc_pts){
////                pt.radius *= 1.5f;
                //swc有负值的要进行转换
//                pt.x = pt.x - bounds.low.x + 1.f;
//                pt.y = pt.y - bounds.low.y + 1.f;
//                pt.z = pt.z - bounds.low.z + 1.f;
//            }
            ...
            LOG_INFO("refined swc bounds is: {} {} {}, {} {} {}", bounds.low.x, bounds.low.y, bounds.low.z,
                     bounds.high.x, bounds.high.y, bounds.high.z);
        }
        if(transform){
            auto space = volume_info.voxel_space * transform_ratio;
            auto _r = space.length() * 0.5;
            for(auto& pt : swc_pts){
//                pt.z += 50.f;
                pt.x *= space.x;
                pt.y *= space.y;
...
#endif
                {
                    AutoTimer _timer("isosurface extracting");
                    for (auto &b : partial_blocks)
                    {
                        ...
                        mesh->Insert(MeshData0(gen_tri_num,
                                               [&](int vert_idx) -> const Float3 & {
...
//            mesh->Smooth(algo_info.smooth_lambda, algo_info.smooth_mu, algo_info.smooth_count);
            ...
            {
                AutoTimer timer("merging");
                mesh->GetPackedMeshDataRef();
            }
            {
